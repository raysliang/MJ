(() => {
    ...
    if (shanten >= 0) {
      for (let type = 0; type < TILE_COUNT; type += 1) {
        ...
        if (nextShanten < shanten) {
          const item = { type, remaining, glyph: tileGlyph(type), name: tileName(type), winning: nextShanten === -1 };
  ...
  function chooseBestDiscard(concealedTiles, melds = [], visibleCounts) {
  ...
  function expectedReplacementQuality(concealedTiles, melds, visibleCounts) {
    ...
    for (let type = 0; type < TILE_COUNT; type += 1) {
      ...
      const analysis = isWinningHand(drawnTiles, melds) ? analyzeHand(drawnTiles, melds, visibleAfterDraw) : chooseBestDiscard(drawnTiles, melds, visibleAfterDraw)?.analysis;
      if (!analysis) {
        continue;
      }
  ...
  function nextTurn(state2) {
    ...
    const seat = state2.players[seatIndex];
    const turnNumber = state2.turn + 1;
    const events = [];
  ...
  var elements = {
    ...
    winningCount: document.querySelector("#winning-count"),
    improvingCount: document.querySelector("#improving-count"),
    winningTiles: document.querySelector("#winning-tiles"),
    improvingTiles: document.querySelector("#improving-tiles"),
  ...
  function renderEastAnalysis(analysis) {
    const improvingTiles = analysis.improvementTiles.filter((item) => !item.winning);
    ...
    elements.analysisNote.textContent = state.activeSeat === 0 && !state.needsDraw && !state.terminal ? "At a discard decision, these counts describe the hand after the recommended throw." : "The counts include East's hand, every discard, and every exposed meld or kong.";
    elements.winningCount.textContent = `${analysis.winningCopies} live ${analysis.winningCopies === 1 ? "copy" : "copies"}`;
    elements.improvingCount.textContent = `${improvingTiles.reduce((total, item) => total + item.remaining, 0)} live ${improvingTiles.reduce((total, item) => total + item.remaining, 0) === 1 ? "copy" : "copies"}`;
    elements.winningTiles.innerHTML = needTilesMarkup(analysis.winningTiles, analysis.complete ? "East already has a complete hand." : "No single draw completes the hand yet.");
    elements.improvingTiles.innerHTML = needTilesMarkup(improvingTiles, analysis.complete ? "No improvements needed." : "No other live tiles improve the hand.");
  }
  function buildTileDecision(option, options, selectedId) {
    ...
    const metrics = `${option.tilesAway} away \xB7 ${option.improvementCopies} live copies \xB7 ${option.improvementTypes} tile types`;
    if (selected) {
      return {
        status: "Chosen throw",
  ...
  function renderBoardStatus() {
    ...
    if (state.terminal) {
      ...
      elements.status.textContent = state.terminal.message;
      elements.status.classList.add("is-terminal");
      elements.next.disabled = true;
    } else {
      elements.boardTitle.textContent = pendingCallDisplay ? `${displayedPlayer.name} just acted` : `${activePlayer.name} to act`;
      elements.status.textContent = pendingCallDisplay ? `${displayedPlayer.name} discarded. ${activePlayer.name} is next.` : state.activeSeat === 0 && state.turn === 0 ? "East is ready with the opening hand." : `${activePlayer.name} ${state.needsDraw ? "draws next" : "must choose a discard"}.`;
      elements.status.classList.remove("is-terminal");
  ...
  function renderHistory() {
    ...
    elements.historyCount.textContent = `${state.history.length} ${state.history.length === 1 ? "move" : "moves"}`;
    if (history.length === 0) {
      elements.historyList.innerHTML = '<li class="history-empty">No decisions yet. The table is waiting for its first turn.</li>';
      return;
    }
    elements.historyList.innerHTML = history.map((entry) => {
      const actor = state.players[entry.seatIndex]?.name ?? "Table";
      const eastAction = entry.seatIndex === 0;
      const eventText = eastAction ? entry.events?.join(" ") ?? "" : "";
      const explanation = eastAction ? entry.explanation : publicHistoryText(entry, actor);
      return `<li class="history-item"><div class="history-item-top"><span class="history-turn">TURN ${entry.turn}</span><strong>${actor}</strong><span class="history-action">${entry.kind}</span></div><p>${explanation}</p>${eventText ? `<small>${eventText}</small>` : ""}</li>`;
    }).join("");
  }
  function publicHistoryText(entry, actor) {
    const discard = entry.discardedTile ? `${actor} discarded ${describeTile(entry.discardedTile)}.` : `${actor} made a public ${entry.kind} action.`;
    if (!entry.call?.kind) {
      return discard;
    }
    const caller = state.players[entry.call.seatIndex]?.name ?? "An opponent";
    const takenTile = entry.call.takenTile ?? entry.discardedTile;
    return `${discard} ${caller} called ${entry.call.kind}${takenTile ? ` on ${describeTile(takenTile)}` : ""}.`;
  }
  function positionTooltip(anchor) {
    ...
    let top = placeAbove ? anchorRect.top - tooltipRect.height - gap : anchorRect.bottom + gap;
