...
function renderBoardStatus() {
  ...
  if (state.terminal) {
    ...
    elements.status.textContent = pendingCallDisplay
      ? `${displayedPlayer.name} discarded. ${activePlayer.name} is next.`
      : state.activeSeat === 0 && state.turn === 0
      ? "East is ready with the opening hand."
      : `${activePlayer.name} ${state.needsDraw ? "draws next" : "must choose a discard"}.`;
...
function publicHistoryText(entry, actor) {
  const discard = entry.discardedTile
    ? `${actor} discarded ${describeTile(entry.discardedTile)}.`
    : `${actor} made a public ${entry.kind} action.`;
  if (!entry.call?.kind) {
    return discard;
  }
